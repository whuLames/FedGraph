...
logging.basicConfig(filename='/home/lames/code/Gcode/FedGraph/log/sage.log',
                    filemode="a",
...
if __name__ == '__main__':
    ...
    server_model = SageLP(data.num_node_features, 128, 64, 32).to(device)

    # model = GCN(data.num_node_features, int(max(data.y)) + 1).to(device)
    model = SageLP(data.num_node_features, 128, 64, 32).to(device)
    # 定义trainer
    ...
    for i in range(round):
        ...
        for i in range(clientnum):
            clientdata = get_data_lp_withextend(data, train_data, i + 1, node_lists)
            #print(clientdata)
