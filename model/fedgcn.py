...
import queue
class FedGcnLayer(MessagePassing):
    ...
    def pass_info(self, data: ClientData, x, new_comm):
        ...
        # for i in range(len(receive_info)):
        #     # buf_size = 200000
        #     # buf = bytearray(buf_size)
        #     # request = comm.irecv(buf)
        #     # status = MPI.Status()
        #     # request.wait(status=status)
        #     # count = status.Get_count()
        #     # msg = bytes(buf[:count])
        #     logging.info('msg 大小 {}'.format(sys.getsizeof(msg)))
        #     msg = comm.recv()
        #     logging.info('msg >>>>>>>>>>>>>>>>>>>>>> {}'.format(msg))
        #     logging.info('已经接受 {} 个 还差 {} 个'.format(i, len(receive_info) - i))
    ...
    def forward(self, data, x, new_comm, Laplace_dict: dict) -> Tensor:
        """
        Paras:
            data: ClientData
...
class FedGCN(nn.Module):
    def __init__(self, INPUT_DIM, OUT_DIM, new_comm) -> None:
        super().__init__()
        # self.conv1 = FedGcnLayer(INPUT_DIM, 32)
        # self.conv2 = FedGcnLayer(32, 16)
        # self.conv3 = FedGcnLayer(16, OUT_DIM)
        self.conv1 = FedGcnLayer(INPUT_DIM, 32)
        self.conv2 = FedGcnLayer(32, OUT_DIM)
    ...
    def forward(self, data, Laplanc_dict=None):
        ...
        x = self.conv1(data, x, new_comm, Laplanc_dict)
        # logging.info('client {} 经过第一层之后的形状为： {}'.format(MPI.COMM_WORLD.Get_rank(), x.shape))
        x = F.relu(x)
        x = F.dropout(x, training=self.training)
        x = self.conv2(data, x, new_comm, Laplanc_dict)
